...
class ArtSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var titleSpr = new FlxSprite().loadGraphic(Paths.image('title', "artgallery"));
		...
		var title = new Alphabet(0, 0, ArtGalleryState.selectedCategoryName);
		title.screenCenter(X);
		for(letter in title.lettersArray) {
			letter.colorTransform.color = 0xffffff;
		}
		title.y -= 30;
		add(title);

		if(ArtGalleryState.selectedCategoryName.contains(" ")) { // Fixes some weird spacing
			titleSpr.scale.x = title.width/(titleSpr.frameWidth-350);
		} else {
			titleSpr.scale.x = title.width/(titleSpr.frameWidth-400);
		}

		credit = new Alphabet(0, 0, "", false, false, 0.05, 0.6);
		add(credit);

		var enter = new AttachedSprite();
		enter.loadGraphic(Paths.image("enter", "artgallery"));
		...
		add(enter);
		support = new Alphabet(0, 0, "Support the artist", false, false, 0.05, 0.6);
		...
		for(letter in support.lettersArray) {
			letter.colorTransform.color = 0xffffff;
		}
		...
		var folder = Paths.getPreloadPath("artgallery/images/" + ArtGalleryState.selectedCategory + "/");
		...
		#if sys
		var infoFile:Array<String> = CoolUtil.coolTextFile('assets/artgallery/images/${ArtGalleryState.selectedCategory}/credits.txt');
		#else
		...
		if(images.length <= 2) {
			...
			for(_week in _origImages) images.push(_week);
			for(_week in _origImages) images.push(_week);
		}
		if(images.length <= 4) {
			var _origImages = images.copy();
		...
		for(_week in origImages) images.push(_week);
		for(_week in origImages) images.push(_week);
		...
		for (i in 0...images.length)
		{
			...
			var weekThing:ArtImage = new ArtImage(0, 0, image, ArtGalleryState.selectedCategory, images[i].endsWith(".jpg"));
		...
		arrowLeft.frames = Paths.getSparrowAtlas("arrows", "artgallery");
		...
		arrowRight.frames = Paths.getSparrowAtlas("arrows", "artgallery");
	...
	override function update(elapsed:Float)
	{
		...
		if(arrowLeft.animation.finished && arrowLeft.animation.name == "press") arrowLeft.animation.play("idle");
		if(arrowRight.animation.finished && arrowRight.animation.name == "press") arrowRight.animation.play("idle");
